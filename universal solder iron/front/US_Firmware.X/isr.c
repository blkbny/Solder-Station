...
void ISRHigh(int src){
    ...
    switch (ISRStep){
        ...
        case 3:
            ...
            if(ISRComplete){
                ...
                if(ADCStep & 1){
                    ...
                    if(OldHeater){
                        UINT32 i, l, cv, ci, sv = 0, si = 0, sp = 0, ri = 0, rv = 0, r = 0;
                        for(i = l =(UINT32)((VIBuffCnt >> 1) - 1); i--;){
                            cv = (VBuff[i] + VBuff[i + 1]);// >> 1;
                            ci = IBuff[i];
                            if((VBuff[i] < 1023) && (VBuff[i + 1] < 1023) && (IBuff[i] < 1023) && (ri < ci)){
                                ri = ci;
                                rv = cv;
                                r = 0;
                            }
        ...
        case 5:
            ...
            if(mainFlags.PowerLost || PV->KeepOff || mainFlags.Calibration)PHEATER = 0;
            HEATER = PHEATER;
            mcuADCStartAuto();
            ShowChar(1);
        ...
        case 7:
            ShowChar(3);
            mcuStartISRTimer(C_PER);
            if(!MAINS || ((VBuff[dw = (mcuIBuffPos() >> 1) - 1] < 90) && (IBuff[dw] < 16))){ //power lost if <0.5A and <7.4V
                OnPowerLost();
                return;
            }
            mcuCompEnableH2L();
