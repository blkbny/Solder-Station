...
void OLEDTasks(){
        ...
        do{
            ...
            switch(CMode){
                ...
                case 5: //Calibration
                    ...
                    CalRes -= CalRes >> 3;
                    //dw=(INT32)PIDVars[0].ADCTemp[0];
                    //dw*=(INT32)33437;
                    //dw/=((INT32)IronPars.Config[0].CurrentA)*((INT32)IronPars.Config[0].Gain);
                    //sdw/=
                    CalRes += ((INT32)PIDVars[0].ADCTemp[0] * (INT32)33437L) / ((INT32)IronPars.Config[0].CurrentA * (INT32)IronPars.Config[0].Gain);
                    break;
                case 0xFF: //stand-by
                    OLEDFlags.f.Message=1;
                    OLEDMsg1="       ZZZ.. .  .";
    ...
    if(OLEDFlags.f.Cal){
        ...
        OLEDPrintNum68(64,3,3,IronPars.Config[0].CurrentA);
        OLEDPrintNum68(32,4,4,PIDVars[0].ADCTemp[0]);
        OLEDPrintNum68(16,5,4,CalRes >> 3);
        OLEDPrintNum68(16,6,5,CompLowTime);
    }
    OLEDUpdate();
}
    ...
    do{
        ...
        switch(CMode){
            case 0: //default mode - display temperature and do nothing
                ...
                if(IronID == 255){
                    DisplayData(s_NoIron);
                }
                else if(ShortCircuit>3){
                    DisplayData(s_HSC);
                }
                else if(NoHeater > 12){
                    DisplayData(s_HErr);
                    ...
                    if((LISRTicks & 15)==1)DisplayInt(DispTemp>>4);
                }
                //if((LISRTicks & 15)==1)DisplayInt(ADCData.VTEMP[2]);
                break;
            case 1: //set temperature mode
                if(BTicks[1].n>100)CMode=0;
                if(BTicks[1].o && (BTicks[1].n==0))CMode=2; //goto reset temperature mode
            ...
            case 3: //set parameter menu
                if(BTicks[0].n | BTicks[1].n | BTicks[2].n){
                    ...
                    if(BTicks[0].n==1)CPar++;
                    if(BTicks[2].n==1)CPar--;
                    if(CPar & 128)CPar=sizeof(pars);
                    if(CPar>=sizeof(pars))CPar=0;
                }
                if(BTicks[1].o &&(BTicks[1].o<100) && (BTicks[1].n==0))CMode=4;
                DisplayData(parText[CPar]);
                break;
            case 4:
                if(BTicks[0].n | BTicks[1].n | BTicks[2].n) ModeTicks=250;
                switch(CPar){
                    case 0:
                    case 1:
                    case 2: //temperature with "off"
                        if(BTicks[0].d){
                            pars.b[CPar]+=1;
                            if(pars.b[CPar]<75)pars.b[CPar]=75;
                            if(pars.b[CPar]>225)pars.b[CPar]=225;
                        }
                        if(BTicks[2].d){
                            if(pars.b[CPar])pars.b[CPar]--;
                            if(pars.b[CPar]<75)pars.b[CPar]=0;
                        }
                        if(pars.b[CPar]){
                            DisplayInt(pars.b[CPar]<<1);
                        }
                        else{
                            DisplayData(s_off);
                        }
                        break;
                    case 3:
                    case 4: //temperature without "off"
                        if(BTicks[0].d)pars.b[CPar]+=1;
                        if(BTicks[2].d)pars.b[CPar]-=1;
                        if(pars.b[CPar]<75)pars.b[CPar]=75;
                        if(pars.b[CPar]>225)pars.b[CPar]=225;
                        DisplayInt(pars.b[CPar]<<1);
                        break;
                    ...
                    case 6: //time
                        if(BTicks[0].d){
                            pars.b[CPar]+=10;
                            if(pars.b[CPar]>240)pars.b[CPar]=240;
                        }
                        if(BTicks[2].d){
                            if(pars.b[CPar]>=10){
                                pars.b[CPar]-=10;
                        ...
                        if(pars.b[CPar]){
                            DisplayInt(pars.b[CPar]);
                        }
                        else{
                            DisplayData(s_off);
                        }
                        break;
                    case 7: //holder sensor
                        if(BTicks[0].d && (pars.Holder<2))pars.Holder++;
                        if(BTicks[2].d && pars.Holder)pars.Holder--;
                        switch(pars.Holder){
                            case 0:
                                DisplayData(s_off);
                                break;
                    ...
                    case 8://display brightness
                        if(BTicks[0].d || BTicks[2].d){
                            if(BTicks[0].d)pars.Bri++;
                            if(BTicks[2].d)pars.Bri--;
                            if(pars.Bri<1)pars.Bri=1;
                            if(pars.Bri>16)pars.Bri=16;
                        }
                        DisplayInt(pars.Bri);
                        break;
                    default:
                ...
                if(BTicks[1].o && (BTicks[1].n==0))CMode=3;
                break;
            case 0xFF: //Standby mode
                ModeTicks=255;
                if(PIDVars->CTemp[0]>100){
                    DisplayData(s_standby_hot);
                }
                else{
                    DisplayData(s_standby);
                }
                break;
            default:
                CMode=0;
...
void MenuTasks(){
    int i;
    if(mainFlags.PowerLost){
        mainFlags.OLED ? OLEDTasks() : LEDTasks();
    }
    else{
        if(LISRTicks != ISRTicks){
            ...
            if((LISRTicks & 1) || mainFlags.PowerLost){
                ...
                if(CMode == 0xFF){
                    ...
                    if((pars.SlTime != 0) && (i >= pars.SlTemp)){
                        switch(pars.Holder){
                            case 1:
                                if(CSeconds >= pars.SlTime) i= pars.SlTemp;
                                break;
                            case 2:
                                if(mainFlags.HolderPresent && (CSeconds >= pars.SlTime)){
                                    i = pars.SlTemp;
                                    break;
                                }
                            default:
                    ...
                    if((pars.STBTime != 0) && (CMinutes >= pars.STBTime))i=0;
                    CTTemp = i;
