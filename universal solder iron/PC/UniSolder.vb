...
#End Region
    ...
    Private Function SendBINCommand(ByRef OutBuffer() As Byte, ByVal OutOffset As UInt32, ByVal OutCount As UInt32, ByRef RespBuffer() As Byte, ByVal TimeOut As UInt32) As Int32
        Dim i As Int32
        Dim rv As Boolean
        Dim OB(63) As Byte
        If OutCount > 0 Then
    ...
    Private Sub TransLayer_DataReceived() Handles TransLayer.DataReceived
        ...
        While TransLayer.RXDataCount > 0
            rlen = TransLayer.Read(RXFBuff, 0, 64)
            If (rlen = 64) Then ProcessRXMessage()
        End While
    End Sub

    Private Function ProcessRXMessage() As Boolean
        Select Case RXFBuff(0)
            Case 3
                RaiseEvent LiveDataReceived(RXFBuff)
            Case Else
                CommandFeedBack.ReadFromBuffer(RXFBuff, 0, 64)
        End Select
        Return True
    End Function
